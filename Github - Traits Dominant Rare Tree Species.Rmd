---
title: "Code - Traits Dominant Rare Tree Species"
author: "Iris Hordijk"
date: "2025-03-01"
...
```
###Identify Dominant and Rare species on plot level
...
pca.df_Traits2 <- data.frame(pca1 = pca_Traits$rotation[,1], pca2 = pca_Traits$rotation[,2],  labels = row.names(pca_Traits$rotation)) 
pca.df_Traits2$col <- c("#050505", "#050505", "#050505", "#050505", "#050505", "#050505", "#050505", "#050505", "#050505", "#050505", "#58038c", "#9e7306", "#1c6915")

#Load data to reproduce figure 1
pca.df_Traits <- fread("~/Documents/PhD projects/2nd Chapter - Trait diversity/pca.df_Traits.csv")
pca.df_Traits2 <-fread("~/Documents/PhD projects/2nd Chapter - Trait diversity/pca.df_Traits2.csv") 
...
Graph_pca_Traits <- ggplot(data=pca.df_Traits, aes(x=pca1, y=pca2)) + geom_point(aes(color = factor(pca.df_Traits$Biome), shape=factor(pca.df_Traits$DominanceCategory)), size = 4, alpha = 0.3) +  scale_color_manual(values = c("#ffdd03", "#8fd613", "#3f5220", "#0281a8", "#00549e", "#270169"), labels = c("Tropical moist", "Tropical dry", "Tropical conifer", "Temperate", "Temperate Conifer", "Boreal")) + 
  labs(x = "PC1 (41%)", y= "PC2 (25%)") + ggtitle("Traits dominant and rare tree species") + theme_bw() + labs(colour="Forest") + 
...
#Load data to reproduce figure 3
TraitsDiffMean <- fread("~/Documents/PhD projects/2nd Chapter - Trait diversity/TraitsDiffMean.csv")
...
#Load data to reproduce figure 4
DifCWM_Envars <- fread("~/Documents/PhD projects/2nd Chapter - Trait diversity/DifCWM_Envars.csv") 

#############Water availability graphs MAIN TEXT
Height_Aridity <- ggplot(DifCWM_Envars, aes(Aridity, Height)) + #linear relationship (quadratic relationship as good)
  geom_smooth(size= 2, colour="#0a0a0a", method="lm") + coord_cartesian(ylim=c(-1, 1)) + theme_classic() + 
...
RootDepth_Aridity <- ggplot(DifCWM_Envars, aes(Aridity, RootDepth)) + #Quadratic relationship (linear relationship as good)
  ...
  xlab(" ") + ylab("Δ Rooting Depth")+ #labs(title="Root Depth") +
  theme(plot.title = element_text(hjust = 0.5, size = 20, face = "bold"),
        axis.title.x = element_text(size = 16, face = "bold"),
        axis.text.x = element_text(size = 14),
        axis.title.y = element_text(size = 16, face = "bold"),
...
pca1_Aridity <- ggplot(DifCWM_Envars, aes(Aridity, pca1)) + #Quadratic relationship with log(y) (linear relationship as good)
  geom_smooth(size= 2, colour="#0a0a0a", method="lm") + xlim(0,4) + coord_cartesian(ylim=c(-1, 1)) + theme_classic() + 
  geom_hline(yintercept=0, color = "grey", size=1) +
  ...
  xlab("Water Availability Index") + ylab("Δ PC1")+ #labs(title="pca1") +
  theme(plot.title = element_text(hjust = 0.5, size = 20, face = "bold"),
        axis.title.x = element_text(size = 16, face = "bold"),
        axis.text.x = element_text(size = 14),
        axis.title.y = element_text(size = 16, face = "bold"),
...
Height_Temperature <- ggplot(DifCWM_Envars, aes(Temperature, Height)) + #Quadratic relationship with log(y) (linear relationship as good)
  geom_smooth(size= 2, colour="#0a0a0a", method="lm", formula=y ~ poly(x, 2, raw=TRUE)) + coord_cartesian(ylim=c(-1, 1)) + theme_classic() + 
...
pca2_Aridity <- ggplot(DifCWM_Envars, aes(Aridity, pca2)) + #linear relationship or quadratic relationship with log(y)
  ...
  geom_segment(aes(x = 1.7, y = 0, xend = 1.7, yend = -Inf), size=0.5, linetype="dashed") +
  xlab("Humidity Index") + ylab("Δ PC2")+ #labs(title="pca2") +
  theme(plot.title = element_text(hjust = 0.5, size = 20, face = "bold"),
        axis.title.x = element_text(size = 16, face = "bold"),
        axis.text.x = element_text(size = 14),
        axis.title.y = element_text(size = 16, face = "bold"),
